...
if not documents:
    ...
    selected_document = get_document(labels[selected_label])
    versions = get_document_versions(selected_document["id"])
    current_content = normalize_meeting_minutes_content(
        selected_document["current_version_content_json"]
    )
    ...
    tab_view, tab_edit, tab_versions, tab_status = st.tabs(
        ["查看內容", "編輯內容", "版本管理", "修改狀態"]
    )
